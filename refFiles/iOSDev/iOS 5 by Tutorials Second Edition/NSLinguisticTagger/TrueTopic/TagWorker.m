  ...
  completion:(TaggingCompletionBlock)block
{
    ...
    NSString* text = [NSString stringWithContentsOfURL: [NSURL URLWithString: url] encoding:NSUTF8StringEncoding
                                                 error:NULL];
    //the list of regexes to cleanup the html content
    NSArray* cleanup = @[@"\\A.*?<body.*?>", //1
                        @"</body>.*?\\Z", //2
                        @"<[^>]+>", //3
                        @"\\W+$"]; //4
    ...
    for (NSString* regexString in cleanup) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:regexString options:NSRegularExpressionDotMatchesLineSeparators error:NULL];
        text = [regex stringByReplacingMatchesInString:text options:NSRegularExpressionDotMatchesLineSeparators range:NSMakeRange(0, [text length]) withTemplate:@""];
    }
    ...
    [tagger enumerateTagsInRange: NSMakeRange(0, [text length]) scheme: NSLinguisticTagSchemeLexicalClass
                         options: NSLinguisticTaggerOmitPunctuation |
     NSLinguisticTaggerOmitWhitespace |
     NSLinguisticTaggerOmitOther | NSLinguisticTaggerJoinNames usingBlock:^(NSString *tag, NSRange tokenRange,
                                                                            NSRange sentenceRange, BOOL *stop)
     {
         ...
         if (text.length== sentenceRange.location+sentenceRange.length) {
             *stop = YES;
             ...
             NSRange resultRange = NSMakeRange(0,(number < words.count)? number:words.count );
