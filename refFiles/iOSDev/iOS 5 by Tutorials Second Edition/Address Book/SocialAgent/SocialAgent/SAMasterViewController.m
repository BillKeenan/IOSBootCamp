...
-(void)importContacts
{
    ...
    dispatch_async(kBgQueue , ^{
        ...
        NSArray* importedPeople = (__bridge_transfer NSArray*) ABPersonCreatePeopleInSourceWithVCardRepresentation( record, (__bridge CFDataRef)responseData); //3
        ...
        for (int i=0;i<[importedPeople count];i++) { //1
            ABRecordRef personRef = (__bridge ABRecordRef)importedPeople[i];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.title = [NSString stringWithFormat: @"%i items",selected.count];
}
